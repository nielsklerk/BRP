...
As you can see in figure \ref, it is hard to visually distinguish the NH3 and NO from the completed spectrum. However, cross-correlation is a technique that can be used to find those smaller signals. In the calculation of the cross-correlation, we use the average of the spectra of all the models in the model grid and normalise them. This template is then used to cross-correlate with the spectrum containing all the models. In figure you can see the cross-correlation of the H2O template with the entire spectrum. The peak signals that water is present in the spectrum. 
\begin{figure}[!ht]
    \centering
...
\textbf{INSERT TABLE OF RANGES}

Using the ranges in table \ref{}. We compared the cross-correlation technique for all the molecules. We did this for the spectrum of the fiducial model. Once, just the spectrum and once the spectrum with the spectrum of the species we are testing is subtracted.

\textbf{removing other species to enhance detection}
\section{Application on Observations}
We apply the techniques of the previous part 
\chapter{Discussion}
pin the end we have a discussion 
\chapter{Conclusion}
